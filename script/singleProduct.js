...
// Função para renderizar a página do produto com base no slug
function renderProductPage(slug) {
	const bodyPdp = document.querySelector(".pdp");

	// Aqui você pode adicionar o código para renderizar a página do produto com base no slug
	// Certifique-se de buscar os dados do produto correspondentes ao slug

	// Exemplo: Renderizar o nome do produto
...
// Função para verificar a URL atual e rotear com base nela
function handleRoutes() {
	const getWindowLocation = window.location.search;
	const params = new URLSearchParams(getWindowLocation);
	const slug = params.get("slug");

	if (slug) {
		// Se houver um slug na URL, renderize a página do produto
		renderProductPage(slug);
	} else {
		// Caso contrário, você pode tratar isso de acordo com sua lógica, como redirecionar para a página inicial
		window.location.href = "index.html";
	}
}

// Chame a função de tratamento de rotas quando a página carregar
